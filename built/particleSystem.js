...
function createParticleSystem (group, particles, scene, time, lastSnapshotTime, settings) {
  ...
  for (let i = particles.length; i < settings.particleCount; i++) {
    ...
    addParticle(new THREE.Color(Math.random(), Math.random(), Math.random()), new THREE.Vector3(x, y, z), new THREE.Vector3(vx, vy, vz), new THREE.Vector3(0, 0, 0), _.random(16, 20, true), _.sample(settings.availableCharges), particles, particlesGeometry, scene, settings.if_showTrajectory, settings.maxTrajectoryLength)
  }
