...
class Particle {
    constructor(index, color, initialPosition, trajectory) {
...
function addParticle(opts) {
    ...
    particleCol.classList.add('particle');
    particleCol.innerText = '⬤';
    particleCol.style.color = color.getStyle();
    row.appendChild(particleCol);
    const massCol = document.createElement('td');
    massCol.classList.add('mass');
    massCol.innerText = `${Math.round(mass * 10) / 10}`;
    row.appendChild(massCol);
    const chargeCol = document.createElement('td');
    chargeCol.classList.add('mass');
    chargeCol.innerText = `${Math.round(charge * 10) / 10}`;
    row.appendChild(chargeCol);
...
function createParticleSystem(group, particles, scene, time, lastSnapshotTime, settings, simState) {
...
function populateFromPreviousOrFresh(particles, particlesGeometry, scene, settings, simState) {
    if (loadState()) {
